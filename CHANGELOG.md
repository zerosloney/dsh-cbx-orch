...
- **工作区徽章与显示修复**：
  - 顶栏左侧将当前工作区药丸（`.ws-pill`）紧随品牌标题与标签呈现，位置更醒目直观。
  - 修复 `loadWorkspaces()` 在初次 401 未登录时失败后无法重试的问题；在 `refresh()` 轮询以及登录成功时自动触发工作区列表重试与渲染，确保工作区名称及任务计数正确显示。
...
- **默认淡色简约风格（Light Minimalist Theme）**：
  - 采用通透浅白背景（`#f8fafc` + 微蓝/浅紫淡光晕）、极细浅灰边框与轻量卡片投影系统。
  - 状态色彩升级为清爽护眼的 Pastel 柔和色板（柔和薄荷青、琥珀暖金、清爽天蓝、珊瑚浅红、玫瑰粉红）。
  - 代码工件区域（Diff / Test / Review / agent.log）升级为 GitHub Light 风格的浅色清晰排版，长时间阅读舒适不刺眼。
  - 顶栏与控制台调整为纯净浅灰与白底卡片容器，保留红黄绿三色精致点缀。
- **主题切换系统（Theme Switcher）**：
  - 顶栏新增主题切换按钮（☀️ 浅色 / 🌙 深色），支持一键无缝热切换。
  - 使用 `localStorage` 自动持久化主题选择偏好，默认呈现极致淡色简约风格。
